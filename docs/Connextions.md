...
1. [Programmatic Connections - api连接](#programmatic)
...
<a name="detaching"></a>
##### Detaching Connections 分离连接
默认情况下 `jsPlumb.connect` 可以用鼠标分离.  一下设置防止此事件:
...
#### Drag and Drop Connections 拖放连接
为了支持拖放连接，每个拖放连接至少需要一个可以从中拖动连接的源端点:
